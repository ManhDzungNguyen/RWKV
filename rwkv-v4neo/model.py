...
class WKV(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, w, u, k, v):
        ...
        assert T <= T_MAX
        if C > 32:
            assert (B * C) % 32 == 0, "Nếu C > 32 thì B * C phải chia hết cho 32"
    ...
    def backward(ctx, gy):
        ...
        if not FLOAT_MODE_fp32:
            gy_ = gy.float()  # biến đổi thành f32
...
class RWKV_TimeMix(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():  # fancy init
            ...
            # fancy time_decay
            decay_speed = [
                -5 + 8 * (h / (attn_sz - 1)) ** (0.7 + 1.3 * ratio_0_to_1)
        ...
        self.time_shift = nn.ZeroPad2d(
            (0, 0, 1, -1)
        )  # padding zero trước embd vector đầu tiên trong batch
